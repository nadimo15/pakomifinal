...
import { initializeDb, getSiteSettings } from './db';
// FIX: The `useAuth` hook is exported from 'hooks/useAuth', not the context file.
import { AuthProvider } from './context/AuthContext';
import { useAuth } from './hooks/useAuth';

// Initialize the local storage database on app load
console.info('App.tsx: Initializing database...');
initializeDb();
console.info('App.tsx: Database initialized.');
...
function App() {
  ...
  useEffect(() => {
    ...
    const handleHashChange = () => {
      console.log(`App.tsx: hashchange event detected. New path: ${getRouteFromHash()}`);
  ...
  useEffect(() => {
    console.log(`App.tsx: Language changed to "${language}". Updating document attributes.`);
  ...
  const navigate = useCallback((path: string) => {
    console.log(`App.tsx: Navigating to "${path}".`);
    window.location.hash = path;
  }, []);

  const renderPage = () => {
    console.log(`App.tsx: Rendering page for route: "${route}"`);
