...
import { getOrders, getChatMessages, addChatMessage, updateOrder, getManagedProducts } from '../db';
...
const TrackOrderPage: React.FC<TrackOrderPageProps> = ({ language, navigate }) => {
  ...
  const handleRefreshStatus = async (orderToRefresh: Order) => {
    ...
    try {
        const company = DELIVERY_COMPANIES.find(c => c.name === orderToRefresh.shippingInfo?.carrier && c.api);
        if (!company) return;
        const { status, lastUpdate } = await trackShipment(orderToRefresh, company);
        const updatedShippingInfo = { ...orderToRefresh.shippingInfo, lastUpdate };
        updateOrder(orderToRefresh.id, { status, shippingInfo: updatedShippingInfo });
        ...
        setError(`Error refreshing status: ${errorMessage}`);
    } finally {
  ...
  const handleTrackOrder = async (idToTrack?: string) => {
    ...
    const foundOrder = allOrders.find(o => o.id.toUpperCase() === finalId);

    if (foundOrder) {
      setOrder(foundOrder);
      setMessages(getChatMessages(foundOrder.id));
      setError('');
      if (!window.location.hash.includes(`id=${foundOrder.id}`)) {
          navigate(`/track?id=${foundOrder.id}`);
      }
      if (['Shipped', 'Pending', 'In Production', 'Designing', 'Printing'].includes(foundOrder.status) && foundOrder.shippingInfo?.trackingNumber) {
        await handleRefreshStatus(foundOrder);
      }
    } else {
    ...
    if (msg.fileUrl) {
      ...
      return (
        <div className="flex flex-col gap-2">
            ...
            <a href={msg.fileUrl} download={msg.fileName} className="flex items-center gap-2 p-2 rounded-lg bg-white/20 hover:bg-white/30 transition-colors">
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="max-w-4xl mx-auto">
          ...
          <form onSubmit={(e) => { e.preventDefault(); handleTrackOrder(); }} className="mt-8 flex gap-2 max-w-lg mx-auto">
            ...
            />
            <button type="submit" className="px-6 py-3 bg-primary text-white font-bold rounded-lg hover:bg-primary-focus transition-colors shadow-md hover:shadow-lg transform hover:-translate-y-0.5">
              {t('track')}
            </button>
          </form>
              ...
              <div className="lg:col-span-3 mt-4 bg-white rounded-xl shadow-lg flex flex-col h-[60vh]">
                ...
                <div className="p-4 border-t bg-white">
                  ...
                  <form onSubmit={handleSendMessage} className="flex items-center gap-3">
                      ...
                      <button type="button" onClick={() => fileInputRef.current?.click()} className="p-3 text-gray-500 hover:text-primary border border-gray-300 bg-white rounded-full transition-colors flex-shrink-0" aria-label={t('attachFile')}>
