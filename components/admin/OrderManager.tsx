...
import type { Language, Order, OrderLineItem, OrderStatus } from '../../types';
import { DELIVERY_COMPANIES, ORDER_STATUSES } from '../../constants';
import ChatModal from './ChatModal';
import ShipOrderModal from './ShipOrderModal';
import OrderEditModal from './OrderEditModal';
import { trackShipment } from '../../api';
import { RefreshCwIcon, ChevronDownIcon, ChevronUpIcon, FacebookIcon, InstagramIcon, TikTokIcon, WhatsAppIcon, ViberIcon, LinkIcon, DownloadIcon, FileIcon, PencilIcon } from '../Icons';
...
const OrderManager: React.FC<OrderManagerProps> = ({ language, orders, onOrderUpdate, onBulkOrderUpdate }) => {
    ...
    const handleRefreshStatus = async (order: Order) => {
        ...
        try {
            const company = DELIVERY_COMPANIES.find(c => c.name === order.shippingInfo.carrier && c.api);
            if (!company) {
                throw new Error('Integrated delivery company not found for this order.');
            }
            const { status, lastUpdate } = await trackShipment(order, company);
            onOrderUpdate(order.id, { 
                status,
                shippingInfo: { ...order.shippingInfo, lastUpdate }
            });
        } catch (error) {
                                    ...
                                    <tr className={`block mb-4 rounded-lg shadow-md md:table-row md:shadow-none md:border-b transition-colors ${selectedOrderIds.has(order.id) ? 'bg-primary/5' : 'bg-white hover:bg-gray-50'}`}>
                                        ...
                                        <td className="px-6 py-2 flex justify-between items-center md:table-cell md:py-4">
                                            ...
                                            <select
                                                ...
                                                className={`w-full max-w-[150px] md:max-w-none text-xs font-semibold p-1.5 rounded border-0 focus:ring-0 ${statusColor(order.status)}`}
                                            >
                                                {ORDER_STATUSES.map(status => (
                                                    <option key={status} value={status}>{t(status.toLowerCase())}</option>
                                                ))}
                                    ...
                                    {isExpanded && (
                                        <tr className="bg-slate-50 border-b md:table-row">
                                            <td colSpan={9} className="p-4 block md:table-cell">
                                                <OrderDetails order={order} t={t} onEditClick={() => handleOpenEdit(order)} />
                                            </td>
                                        </tr>
                                    )}
                                </React.Fragment>
...
const OrderDetails: React.FC<{order: Order, t: (key: string) => string, onEditClick: () => void}> = ({ order, t, onEditClick }) => {
        ...
        return (
            <div className="flex flex-col sm:flex-row gap-4 p-3 bg-slate-100 rounded-lg">
                <div className="w-20 h-20 bg-white rounded-md flex-shrink-0 flex items-center justify-center p-1 border mx-auto sm:mx-0">
                    <img src={product?.galleryImagesB64[0]} alt={item.productName} className="max-w-full max-h-full object-contain" />
                </div>
                    ...
                    <div className="flex-shrink-0 w-full sm:w-24 text-center border-t sm:border-none pt-2 sm:pt-0">
                        {isImage ? (
                            ...
                            <div className="flex flex-col items-center text-gray-500">
                                ...
                                <p className="text-xs truncate">{logoMime}</p>
                            </div>
                        )}
            ...
            {Object.values(order.socials).some(val => Array.isArray(val) ? val.length > 0 : !!val) && (
                <div>
                    ...
                    <div className="flex flex-wrap gap-x-4 gap-y-2 text-sm">
                        ...
                        {order.socials.others?.map((other, i) => (
                             <a key={i} href={other.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-gray-700 hover:text-primary">
                                <LinkIcon className="w-5 h-5" /> {other.url}
                             </a>
                        ))}
                    </div>
                </div>
