...
const CheckoutPage: React.FC<CheckoutPageProps> = ({ language, navigate }) => {
    ...
    const [formConfig, setFormConfig] = useState<FormConfig['yourDetails'] | null>(null);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const newOrder = await addOrder(clientDetails, cartItems, currentUser?.id);
            clearCart();
                ...
                <form id="checkout-form" onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    <div className="lg:col-span-2 bg-white p-6 rounded-lg shadow-md mb-40 lg:mb-0">
                        ...
                        <div className="space-y-4">
                            {getField('clientName')?.enabled && <div> <label className="block text-sm font-medium text-gray-600 mb-1">{t('clientName')}{getField('clientName')?.required && <span className="text-red-500">*</span>}</label> <input type="text" value={clientDetails.clientName} onChange={e => handleDetailChange('clientName', e.target.value)} required={getField('clientName')?.required} className="w-full p-2 border rounded"/> </div>}
                            {getField('emailAddress')?.enabled && <div> <label className="block text-sm font-medium text-gray-600 mb-1">{t('emailAddress')}{getField('emailAddress')?.required && <span className="text-red-500">*</span>}</label> <input type="email" value={clientDetails.email} onChange={e => handleDetailChange('email', e.target.value)} required={getField('emailAddress')?.required} className="w-full p-2 border rounded"/> </div>}
                            {getField('phone')?.enabled && <div> <label className="block text-sm font-medium text-gray-600 mb-1">{t('phone')}{getField('phone')?.required && <span className="text-red-500">*</span>}</label> <input type="tel" value={clientDetails.phone} onChange={e => handleDetailChange('phone', e.target.value)} required={getField('phone')?.required} className="w-full p-2 border rounded"/> </div>}
                            {getField('wilaya')?.enabled && <div> <label className="block text-sm font-medium text-gray-600 mb-1">{t('wilaya')}{getField('wilaya')?.required && <span className="text-red-500">*</span>}</label> <select value={clientDetails.wilaya} onChange={e => handleDetailChange('wilaya', e.target.value)} required={getField('wilaya')?.required} className="w-full p-2 border rounded bg-white"> <option value="">{t('selectWilaya')}</option> {WILAYAS.map(w => <option key={w.id} value={w.name}>{w.id} - {w.name}</option>)} </select> </div>}
                            {getField('commune')?.enabled && <div> <label className="block text-sm font-medium text-gray-600 mb-1">{t('commune')}{getField('commune')?.required && <span className="text-red-500">*</span>}</label> <select value={clientDetails.commune} onChange={e => handleDetailChange('commune', e.target.value)} required={getField('commune')?.required} disabled={!clientDetails.wilaya} className="w-full p-2 border rounded bg-white disabled:bg-gray-100"> <option value="">{t('selectCommune')}</option> {availableCommunes.map(c => <option key={c.id} value={c.name}>{c.name}</option>)} </select> </div>}
                            {getField('address')?.enabled && <div> <label className="block text-sm font-medium text-gray-600 mb-1">{t('address')}{getField('address')?.required && <span className="text-red-500">*</span>}</label> <input type="text" value={clientDetails.address} onChange={e => handleDetailChange('address', e.target.value)} required={getField('address')?.required} className="w-full p-2 border rounded"/> </div>}
