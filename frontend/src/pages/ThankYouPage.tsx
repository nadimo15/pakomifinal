...
import { getOrderById, getManagedProducts, getSiteSettings } from '../api';
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const [siteSettings, foundOrder] = await Promise.all([
            getSiteSettings(),
        ...
        if (foundOrder && siteSettings.upsell.enabled) {
          ...
          console.error("Failed to load thank you page data:", error);
      } finally {
          setIsLoading(false);
      }
    };
  ...
  if (!order || !settings) {
    return (
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-20 text-center">
            ...
            <p className="text-gray-600 mt-2">We couldn't find the order details. Please verify your order ID or contact support.</p>
        </div>
    );
  }
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="max-w-2xl mx-auto bg-white p-8 rounded-xl shadow-lg text-center animate-fade-in-up">
            ...
            <div className="mt-6">
                ...
                <button onClick={handleCopy} className="w-full sm:w-auto bg-primary text-white font-bold py-3 px-6 rounded-lg hover:bg-primary-focus transition-colors">
                    ...
                    {upsellProducts.map(product => (
                        <div key={product.id} className="bg-white rounded-lg shadow-md overflow-hidden text-center transition-transform transform hover:scale-105">
                            <div className="w-full h-48 flex items-center justify-center p-4 bg-gray-100">
                                <img src={(product.galleryImagesB64 as string[])[0]} alt={product.name} className="max-h-full max-w-full object-contain" />
                            </div>
                            <div className="p-6">
